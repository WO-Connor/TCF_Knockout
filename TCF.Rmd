...
```{r integrate}
seurat_integrated <- normalize_integrate(filtered_seurat,c("nCount_RNA","nFeature_RNA"),3000)
...
```{r load_hep_data}
#seurat_integrated <- readRDS("TCF_Knockout_seurat-integrated.rds")
hepatocyte_obj <- subset(x = seurat_integrated, subset = `seurat_clusters` ==  c('0'))
...
hepatocyte_obj$dpt <- ((dpt_vals)/max(dpt_vals))*8+1
...
```{r plot_dpt}
plot_gene_dpt_list(hepatocyte_obj,genes$Pericentral[!genes$Pericentral%in%c("Axin","Gpr49",NA)])
plot_gene_dpt_list(hepatocyte_obj,genes$Periportal)
```
...
plot_gene_dpt_cumulative(hepatocyte_obj,genes$Pericentral[!genes$Pericentral%in%c("Axin","Gpr49",NA)],"Pericentral")
plot_gene_dpt_cumulative(hepatocyte_obj,genes$Periportal,"Periportal")
```
