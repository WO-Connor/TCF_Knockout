...
phm <- pheatmap(heatmapm,
         color = colorRampPalette(c(paste0("#",c1),'#F6F6F6',paste0("#",c2)))(255),
         scale = 'row',
         fontsize = 16,
         fontsize_row = 10,
         fontsize_col = 10,
         ...
         annotation_names_row = F,
         main = "WT                              KO",
         filename = paste0("zonated_binned_dpt_evenbins_correctgenes_heatmap_012_4_",c1,"_",c2,".png"),
...
pheatmap(heatmap,
         # color = colorRampPalette(c('#408697','#F6F6F6','#8F0009'))(255),
         color = colorRampPalette(c('#994f00','#F6F6F6','#005ab5'))(255),
         scale = 'row',
         fontsize = 16,
         fontsize_row = 10,
         fontsize_col = 10,
         ...
         treeheight_row = 50,
         main = "WT                              KO",
         filename = "filtered_10pctanybin_finallist_Gln_Heatmap_purpleorange.png",
...
df <- data.frame(t(df))
colnames(df) <- colnm
df$bdpt <- hepatocyte_obj_reint$binneddptgeno
table(df$bdpt)
...
expr$X <- as.numeric(substring(expr$bdpt,4))-1
expr$geno <- substring(expr$bdpt,1,2)
expr$pct <- 1
expr$sepct <- 1
...
write.csv(expr,"Pck1_8bin_DPT.csv")
...
expr[expr$geno=='WT',]$geno <- "CON"
expr[expr$geno=='KO',]$geno <- "L-KO"
write.csv(expr,"G6pc_8bin_DPT.csv")
...
for(i in findgenes(hepatocyte_obj_reint,"Slc2a")){
  ...
  expr[expr$geno=='WT',]$geno <- "CON"
  expr[expr$geno=='KO',]$geno <- "L-KO"
  write.csv(expr,paste0(i,"_8bin_DPT.csv"))
}
```
